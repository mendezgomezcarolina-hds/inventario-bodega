...
function stockHistorico(e) {
  try {
    ...
    // Reconciliar por (lugar, código): cualquier EGRESO/AJUSTE- cuya fecha
    // de vencimiento no calzó EXACTAMENTE con un lote real (sin fecha, o con
    // una fecha ligeramente distinta por error de tipeo — ej. "30-08-2026"
    // en vez de "31-08-2026") queda en un grupo aparte que puede quedar
    // negativo, mientras el lote real sigue mostrando su cantidad completa
    // sin descontar. Se descuenta ese faltante de los lotes reales (los que
    // terminaron con saldo positivo) más antiguos primero (FEFO).
    ...
    for (var lc in porLugarCod) {
      ...
      var keysNegativos = keys.filter(function(k){ return grupos[k].cantidad < 0; });
      var keysPositivos = keys.filter(function(k){ return grupos[k].cantidad > 0; });
      if (!keysNegativos.length || !keysPositivos.length) continue;

      keysPositivos.sort(function(a, b) {
        var da = grupos[a].vencDate ? grupos[a].vencDate.getTime() : Infinity;
        var db = grupos[b].vencDate ? grupos[b].vencDate.getTime() : Infinity;
        return da - db;
      });
      keysNegativos.forEach(function(kn) {
        var faltante = -grupos[kn].cantidad;
        for (var li = 0; li < keysPositivos.length && faltante > 0.0001; li++) {
          var lote = grupos[keysPositivos[li]];
          if (lote.cantidad <= 0) continue;
          var descuento = Math.min(lote.cantidad, faltante);
          lote.cantidad -= descuento;
          faltante -= descuento;
        }
        grupos[kn].cantidad = 0; // el grupo desajustado queda saldado, no se muestra
      });
    }
