...
// Nota: Mes, N°Sol y Lugar pueden estar solo en la primera fila del grupo
function listarRecepcion(e) {
  try {
    ...
    // Propagar Mes, N°Sol y Lugar a filas vacías del mismo grupo
    var mesCurrent  = "";
    var idCurrent   = "";
    var lugarCurrent = "";
    var respCurrent  = "";
    ...
    for (var i = inicio; i < datos.length; i++) {
      ...
      // Si todas las celdas clave están vacías, fin del bloque
      ...
      // Solo traer pendientes (vacío o "PENDIENTE")
      if (estadoFila !== "PENDIENTE") continue;

      // Necesita al menos descripcion o codigo
      if (!f[3] && !f[4]) continue;
      ...
      pedidos.push({
        ...
        codigo:         String(f[3] || ""),
        item:           String(f[4] || ""),
        cantSolicitada: String(f[5] || "").replace("-","").trim(),
        cantRecibida:   String(f[6] || "").replace("-","").trim(),
